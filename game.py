...
class Game():
    ...
    def gameloop(self):
        while self.playing:
            ...
            # else:
            #     surface2 = pygame.Surface((pygame.display.Info().current_h, pygame.display.Info().current_w))
            #     surface2.fill('white')
            #     surface2.set_alpha(200)
            #     surface2.blit(image, (0,0))
            #     surface.blit(surface2, rect, self.imagerect)
            
            
            
            
            # color = self.board.game.get_color(1) if self.id == self.board.size**2 else self.board.game.get_color(2)
            # surface.blit(self.board.game.image, rect, (temp_x, temp_y, self.board.game.get_tile_size(), self.board.game.get_tile_size())) 
            # color = self.board.game.white
            # surface.blit(rect)
